        ...
        <form action="{{ route('notes.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required
                       class="mt-1 block w-full border border-gray-300 rounded p-2 focus:border-blue-500 focus:ring focus:ring-blue-200" placeholder="Masukkan judul...">
            </div>

            <div>
                <label for="content" class="block text-gray-700">Isi:</label>
                <textarea name="content" required
                          class="mt-1 block w-full border border-gray-300 rounded p-2 focus:border-blue-500 focus:ring focus:ring-blue-200" placeholder="Masukkan isi catatan...">{{ old('content') }}</textarea>
            </div>
